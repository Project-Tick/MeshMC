...
/*
 * Bridges an account's Minecraft skin face into QML, the same way
 * InstanceIconProvider (see InstanceIconProvider.h) bridges IconList. A
 * delegate writes
 *
 *     Image { source: "image://accountface/" + accountId }
 *
 * where accountId is a profile id or an account's internalId() -- whichever
 * QmlShell ends up exposing as the default account's identifier.
 *
 ...
 * requestedSize falls back to a fixed 64x64. The face is scaled with
 * Qt::FastTransformation (nearest-neighbour), not the smooth default: this
 * is 8x8 pixel art, and smooth-scaling it up would blur the very pixel edges
