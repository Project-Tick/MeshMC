...
FileIgnoreProxy::FileIgnoreProxy(QString root, QObject* parent)
	: QSortFilterProxyModel(parent), m_root(std::move(root))
{
}

bool FileIgnoreProxy::lessThan(const QModelIndex& left,
...
bool FileIgnoreProxy::setData(const QModelIndex& index, const QVariant& value,
							  int role)
{
	if (index.column() == 0 && role == Qt::CheckStateRole) {
...
bool FileIgnoreProxy::filterAcceptsColumn(
...
bool FileIgnoreProxy::filterAcceptsRow(int source_row,
									   const QModelIndex& source_parent) const
{
	...
	QModelIndex index = fsm->index(source_row, 0, source_parent);
	return !ignoreFile(fsm->fileInfo(index));
}

bool FileIgnoreProxy::ignoreFile(const QFileInfo& file) const
{
	if (m_ignoreFiles.contains(file.fileName())) {
		return true;
	}
	...
	if (m_ignoreFilePaths.covers(relPath(file.absoluteFilePath()))) {
		return true;
	}
...
{
	return m_blocked.covers(relPath(file.absoluteFilePath())) ||
		   ignoreFile(file);
}
