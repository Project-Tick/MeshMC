...
OUTPUT="$REPO/build-translations"
LOCALE_DIR="$REPO/launcher/locale"
...
for po_file in "$LOCALE_DIR"/*.po
do
    echo "Considering ${po_file}"
    if grep '\"X-Qt-Contexts: true\\n\"' "$po_file" > /dev/null; then
